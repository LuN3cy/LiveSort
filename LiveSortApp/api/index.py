...
TEMP_DIR = Path("/tmp") if IS_VERCEL else BASE_DIR

# Define paths
MUSIC_DIR = TEMP_DIR / "music_files"
CACHE_FILE = TEMP_DIR / "analysis_cache.json"
...
# These are part of the deployment, should be read-only but existing
STATIC_DIR = BASE_DIR / "static"
TEMPLATES_DIR = BASE_DIR / "templates"

# Mount static files
app.mount("/static", StaticFiles(directory=str(STATIC_DIR)), name="static")
app.mount("/music_files", StaticFiles(directory=str(MUSIC_DIR)), name="music_files")
...
def _list_audio_filenames(music_dir: str | Path) -> list[str]:
...
async def upload_files(
    ...
    for file in files:
        if file.filename:
            ...
            if ext == ".ncm":
                try:
                    converted_filename = _decrypt_ncm_file(file_path, str(music_dir))
                    os.remove(file_path)
...
def _encode_mp3_with_progress(
    ...
    if return_code != 0:
        raise RuntimeError((stderr_output or "ffmpeg failed").strip())
    if progress_callback is not None:
        progress_callback(end_progress, "encoding", "正在完成 MP3 导出")
